...
class Layer(object):
    ...
    def get_updates(self, cost, optimizer):
        updates = list((p, optimizer(p, T.grad(cost=cost, wrt=p))) for p in attrs_of(self, SharedVariable))
        # updates += list(chain.from_iterable(l.get_updates(cost, optimizer) for l in attrs_of(self, Layer)))
        return updates
...
class GRU(Layer):
    def __init__(self, input_dim, output_dim, input_layer, s0=None, name=""):
        ...
        states, self._updates = theano.scan(
            fn=step,
            sequences=[self.X],
    ...
    def get_updates(self, cost, optimizer):
        grads = super().get_updates(cost, optimizer)
        return grads + [self._updates]
...
class LSTM:
    def __init__(self, input_dim, output_dim, input_layer, s0=None, c0=None,
        ...
        self.c0 = c0
        if self.s0 is None:
            self.s0 = T.zeros([self.X.shape[1], output_dim])  # (n_batch, n_output)
        if self.c0 is None:
            self.c0 = T.zeros([self.X.shape[1], output_dim])  # (n_batch, n_output)
        self.params = []
        ...
        states, self.other_updates = theano.scan(
            fn=step,
            sequences=[self.X],
            outputs_info=outputs_info
            )
